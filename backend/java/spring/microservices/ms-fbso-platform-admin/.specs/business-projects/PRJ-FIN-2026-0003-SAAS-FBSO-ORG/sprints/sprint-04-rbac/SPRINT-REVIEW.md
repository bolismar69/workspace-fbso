...
- **Status atual:** Frentes 0-4 ✅ concluídas (17/07/2026) · Frentes 5-5b ⬜ pendentes
...
- [x] **Atribuir:** Vincular usuário a BU-A com papel Gerente
- [x] **Revogar:** Remover permissão → efeito imediato na próxima requisição (RN11-03)
- [x] **API:** GET /users/{uid}/permissions retorna vínculos atuais
- [x] **API:** PUT /users/{uid}/permissions atualiza vínculos com auditoria
- [x] **Isolamento:** Usuário BU-A vê apenas produtos da BU-A
- [x] **Tentativa de acesso:** Usuário BU-A tenta acessar produto BU-B por ID direto → 404

---

### Frente 4 ✅ — Integração RBAC + Segurança (F03-04) — CONCLUÍDA 17/07/2026

#### Demonstração por Papel

- [x] **Login como Admin Tenant:** Acessa tudo — dashboard, tenants, planos, usuários, permissões, BUs, produtos
- [x] **Login como Gerente BU:** Vê BUs e produtos da sua unidade. Pode criar/editar. Não vê tenants, planos, auditoria
- [x] **Login como Operador BU:** Apenas leitura de BUs e produtos. Tentar editar → 403
- [x] **Login como Auditor:** Apenas leitura de auditoria. Tentar criar qualquer coisa → 403

> 🎬 **Script:** "Vou fazer login como cada um dos 4 papéis. Reparem que o menu lateral muda. O Operador tenta criar um produto... 403."

#### 403 Amigável

- [x] **Acesso direto:** Operador digita URL `/admin/plans/create` → 403
- [x] **Formato:** `{"type":"...","title":"Acesso negado","detail":"Você não tem permissão para acessar esta área.","status":403}`
- [x] **Sem vazamento:** 403 (nunca 404 — não revela existência do recurso, RN12-01)
- [x] **Sem stack trace:** Nenhum detalhe técnico na resposta

#### Testes

- [x] **Unitários:** UserService, PermissionService ≥ 80% cobertura
- [x] **Parametrizado:** 41 combinações papel × endpoint proibido → 403
- [x] **Segurança:** Matriz RN10-01 100% validada. RbacAspect DB-backed sem fallback JWT
...
| Frente 1 — Gestão de Usuários | 3 | 3 ✅ | 0 |
| Frente 2 — Entidades RBAC | 2 | 2 ✅ | 0 |
| Frente 3 — PermissionService + API | 2 | 2 ✅ | 0 |
| Frente 4 — Integração + Testes | 4 | 4 ✅ | 0 |
