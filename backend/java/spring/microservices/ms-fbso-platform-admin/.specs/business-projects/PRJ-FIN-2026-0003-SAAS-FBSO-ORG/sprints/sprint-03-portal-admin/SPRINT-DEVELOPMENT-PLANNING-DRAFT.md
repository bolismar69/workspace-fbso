...
- **Duração efetiva:** ~12 dias úteis de desenvolvimento
- **Status:** 🔴 Não iniciada — planejamento prévio
- **Documentos-mestre:** [TASKS.md](../TASKS.md) v2.4 · [SPECS.md](../SPECS.md) v1.6 · [TEST_PLAN.md](../TEST_PLAN.md) v2.4 · [PRD.md](../PRD.md) v1.5 · [ARCHITECTURE.md](../ARCHITECTURE.md) v2.0
...
| Regras de negócio | 20 (RN01-01 a RN08-02) |
| Endpoints REST | 17 novos |
| Cenários de teste | 55 (24 unit + 19 integração + 6 E2E + 6 segurança) |
...
| **5** | Entidades M3 | T-024 (Tenant), T-029 (Plan), T-032 (Subscription) | 3 entidades + DTOs |
...
> **Nota:** Dias 1-12 são dias úteis de desenvolvimento, não dias corridos. Com 1 desenvolvedor full-time, a sprint levaria ~3 semanas. Com 2 desenvolvedores, ~2 semanas.
...
| **D5** | Índices para performance do dashboard | `idx_tenant_status`, `idx_tenant_created`, `idx_tenant_plan` | Criar índices na **Migration V004** (antes de T-016) para queries agregadas SEM full scan | Performance (meta ≤3s) |
...
### 4.2 Migration V004 — Índices de Performance

**PRÉ-REQUISITO para T-016.** Criar antes de qualquer query de dashboard:
...
CREATE INDEX idx_tenant_created ON fbso_platform.tenant (created_dt) WHERE deleted_dt IS NULL;
CREATE INDEX idx_tenant_segment ON fbso_platform.tenant (segment) WHERE deleted_dt IS NULL;
CREATE INDEX idx_subscription_tenant ON fbso_platform.subscription (tenant_id, status);
CREATE INDEX idx_audit_timestamp ON fbso_platform.audit_log (timestamp DESC);
CREATE INDEX idx_audit_entity ON fbso_platform.audit_log (entity_type, action);
```
...
| **Integração** | Testcontainers + PostgreSQL 17 | 19 | Repositories com queries reais. Cenários de borda (409, 422, 403) |
...
| **Sprint 3 concluída** | ~Dia 12 | 23/23 tasks. 55/55 testes. Cobertura ≥ 80%. Demo PO | Sprint review checklist preenchido |
...
- [ ] **Testes:** 55/55 cenários passando (24 unit + 19 int + 6 E2E + 6 seg)
...
> Esta seção deve ser preenchida durante a sprint conforme decisões são tomadas.
...
- [ ] Ambiente Docker funcional (postgres + keycloak + mailhog)
- [ ] Migration V004 (índices) criada e aplicada
- [ ] Script de seed com 50+ tenants pronto
- [ ] `mvn clean compile` passa
- [ ] 22 testes da Sprint 2 continuam passando
- [ ] Decisões D1-D6 discutidas e registradas
