# PROMPT-GENERATE-560-SPECS-DEFINITION (F16)

## Contexto

Este prompt gera o artefato `560-SPECS-DEFINITION.md` — a **consolidação técnica enxuta** do projeto. Diferentemente de uma baseline exaustiva, este documento **sumariza e referencia** os artefatos técnicos já produzidos nos blocos anteriores, funcionando como um índice navegável que evita duplicação de conteúdo.
...
Gerar `{TECHNICAL_DEFINITIONS_PATH}/560-SPECS-DEFINITION.md` com as seguintes 10 seções. **Cada seção deve conter ~1 parágrafo de sumário + `→ ver [ARTEFATO]` para detalhes. NÃO duplicar conteúdo dos artefatos referenciados.**
