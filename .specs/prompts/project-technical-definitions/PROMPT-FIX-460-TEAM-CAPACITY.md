# PROMPT-FIX-460-TEAM-CAPACITY

## Contexto

Este prompt é acionado quando o `PROMPT-GATE-460-TEAM-CAPACITY.md` emite `[NÃO COMPLIANCE]` para o artefato `460-TEAM-CAPACITY.md` (Bloco A — Fase 6). O agente corretor aplica correções cirúrgicas com base no relatório inline do gate. **Nunca reescreve o documento do zero. Modifique estritamente as seções, tabelas ou linhas apontadas como Não Compliance.**
...
Ler o **Relatório de Auditoria** emitido pelo gate (relatório inline com os IDs de conflito e respostas do humano), `460-TEAM-CAPACITY.md` (artefato a corrigir) e `450-TEAM-SKILLS-MAP.md` (referência de papéis).
