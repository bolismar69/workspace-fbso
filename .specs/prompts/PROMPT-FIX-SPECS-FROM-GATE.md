...
Este prompt é acionado quando o **Gate de Viabilidade e Completude Técnica** (PROMPT-GATE-SPECS-TECHNICAL) reprova o `SPECS.md` e gera o relatório `TECHNICAL_SPECS_FAIL_REPORT.md`.

O agente atua como **corretor de especificações** — lê o relatório de falha e aplica correções cirúrgicas no SPECS.md, priorizando itens **bloqueantes** (que impedem a geração de TASKS.md) sobre não-bloqueantes.

**Princípio fundamental:** O SPECS.md é a ponte negócio→técnico. Correções aqui impactam diretamente a qualidade das tarefas e dos testes downstream.
...
Ler obrigatoriamente:
    ...
    ├── {SOLUTION_PATH}/.specs/business-projects/{PROJECT_NAME}/SPECS.md (a corrigir)
...
### Passo 3 — Aplicar Correções no SPECS.md
...
Para cada NC no relatório:
    ...
    │         → Consultar feature no 04-FEATURES.md e PRD.md
...
| 6 | Consistência cross-documento | SPECS.md alinhado com PRD.md e ARCHITECTURE.md |
...
| 5ª | `documentation-writer` | Qualidade e consistência do SPECS.md corrigido |
...
2. **Correções cirúrgicas, mantendo o escopo.** Não adicionar features, APIs ou regras de negócio que não estejam no PRD.md. Se o PRD.md estiver desatualizado, corrigi-lo primeiro (via PROMPT-FIX-PRD-FROM-GATE).
...
5. **Após correção, reexecutar o gate.** FIX SPECS → GATE SPECS → (se APROVADO) prosseguir para TASKS.md.
...
|:---|:---|:---|:---|
| 1.0 | 13/07/2026 | Criação inicial: correção de SPECS.md baseada em TECHNICAL_SPECS_FAIL_REPORT.md, 4 níveis de prioridade, foco em itens bloqueantes | Time de Arquitetura |
