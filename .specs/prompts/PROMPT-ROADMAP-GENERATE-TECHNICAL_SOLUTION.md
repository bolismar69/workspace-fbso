# PROMPT: ROADMAP DE EXECUÇÃO MACRO E GUIA DE ORQUESTRAÇÃO DE DOCUMENTOS — SOLUÇÃO TÉCNICA
...
|---|---------|------------------|--------|-------------|
| 1 | PRD.md | `{SPECS_PATH}/PRD.md` | ✅ Existe (N linhas) / ❌ Não existe | ✅ Compliance / ⚠️ Pendente / — |
| 2 | ARCHITECTURE.md | `{SPECS_PATH}/ARCHITECTURE.md` | ✅ Existe (N linhas) / ❌ Não existe | ✅ Compliance / ⚠️ Pendente / — |
| 3 | SECURITY.md | `{SPECS_PATH}/SECURITY.md` | ✅ Existe (N linhas) / ❌ Não existe | ✅ Compliance / ⚠️ Pendente / — |
| 4 | SPECS.md | `{SPECS_PATH}/SPECS.md` | ✅ Existe (N linhas) / ❌ Não existe | ✅ Compliance / ⚠️ Pendente / — |
| 5 | TASKS.md | `{SPECS_PATH}/TASKS.md` | ✅ Existe (N linhas) / ❌ Não existe | ✅ Compliance / ⚠️ Pendente / — |
| 6 | TEST_PLAN.md | `{SPECS_PATH}/TEST_PLAN.md` | ✅ Existe (N linhas) / ❌ Não existe | ✅ Compliance / ⚠️ Pendente / — |

**Lógica de decisão com base no status:**

- Se **todos** os arquivos estão marcados como ❌ Não existe → Solução nova. Iniciar da Fase 1 (PRD.md).
...
|---|---|
| PRD.md | ARCHITECTURE.md → SECURITY.md → SPECS.md → TASKS.md → TEST_PLAN.md |
| ARCHITECTURE.md | SECURITY.md → SPECS.md → TASKS.md → TEST_PLAN.md |
| SECURITY.md | SPECS.md → TASKS.md → TEST_PLAN.md |
| SPECS.md | TASKS.md → TEST_PLAN.md |
| TASKS.md | TEST_PLAN.md |
...
- **Entregável:** Arquivo `PRD.md` em `{SPECS_PATH}/PRD.md` validado pelo humano com marcador `[STATUS: COMPLIANCE]`
- **Pipeline Sequencial:**
  1. Executar `PROMPT-GENERATE-PRD-TECHNICAL_SOLUTION.md` com parâmetros: `{SOLUTION_PATH=TECHNICAL_SOLUTION_COMPLETE_PATH_NAME}`, `{PROJECT_PATH=PROJECT_COMPLETE_PATH_NAME}`, `{PROJECT_NAME=PROJECT_ID_NAME}`, `{SOLUTION_NAME=TECHNICAL_SOLUTION_NAME}`, `{STACK=STACK_DEFINITION}`, `{SCOPE=full}`
  2. Validar via `PROMPT-GATE-PRD-TECHNICAL_SOLUTION.md`
  3. Aplicar loops de correção (`PROMPT-FIX-PRD-TECHNICAL_SOLUTION.md`) ou loops de retrocesso por novos insumos conforme o Mecanismo de Orquestração até o aceite final e explícito do humano.

---

### Fase 2 — ARCHITECTURE.md (Documento de Arquitetura)
...
- **Inputs:**
  - `PRD.md` validado (Fase 1 — obrigatório)
  - `{ARCHITECTURE_GLOBAL}` — ADRs globais, blueprints, governança, data standards (obrigatório)
  - `TECHNICAL-PLAN.md` e `ARCHITECTURE.md` do projeto de negócio (se existirem)
- **Checkpoint de Rastreabilidade:** Validar se cada decisão arquitetural (ADR) atende diretamente a um requisito não-funcional (NFR) ou funcionalidade declarada no PRD.md.
- **Responsáveis:** Tech Lead / Arquiteto de Solução
- **Entregável:** Arquivo `ARCHITECTURE.md` em `{SPECS_PATH}/ARCHITECTURE.md` validado pelo humano com marcador `[STATUS: COMPLIANCE]`
- **Pipeline Sequencial:**
  1. Executar `PROMPT-GENERATE-ARCHITECTURE-TECHNICAL_SOLUTION.md` com parâmetros: `{SOLUTION_PATH=TECHNICAL_SOLUTION_COMPLETE_PATH_NAME}`, `{PROJECT_PATH=PROJECT_COMPLETE_PATH_NAME}`, `{PROJECT_NAME=PROJECT_ID_NAME}`, `{SOLUTION_NAME=TECHNICAL_SOLUTION_NAME}`, `{STACK=STACK_DEFINITION}`
  2. Validar via `PROMPT-GATE-ARCHITECTURE-TECHNICAL_SOLUTION.md`
  3. Aplicar loops de correção (`PROMPT-FIX-ARCHITECTURE-TECHNICAL_SOLUTION.md`) ou retrocesso até o aceite humano final.

---

### Fase 3 — SECURITY.md (Documento de Segurança) 🆕
...
- **Inputs:**
  - `PRD.md` validado (Fase 1 — obrigatório)
  - `ARCHITECTURE.md` validado (Fase 2 — obrigatório)
...
- **Entregável:** Arquivo `SECURITY.md` em `{SPECS_PATH}/SECURITY.md` validado pelo humano com marcador `[STATUS: COMPLIANCE]`
- **Pipeline Sequencial:**
  1. Executar `PROMPT-GENERATE-SECURITY-TECHNICAL_SOLUTION.md` com parâmetros: `{SOLUTION_PATH=TECHNICAL_SOLUTION_COMPLETE_PATH_NAME}`, `{PROJECT_PATH=PROJECT_COMPLETE_PATH_NAME}`, `{PROJECT_NAME=PROJECT_ID_NAME}`, `{SOLUTION_NAME=TECHNICAL_SOLUTION_NAME}`, `{STACK=STACK_DEFINITION}`, `{SECURITY_GLOBAL=SECURITY_GLOBAL}`
  2. Validar via `PROMPT-GATE-SECURITY-TECHNICAL_SOLUTION.md`
  3. Aplicar loops de correção (`PROMPT-FIX-SECURITY-TECHNICAL_SOLUTION.md`) ou retrocesso até o aceite humano final.

---

### Fase 4 — SPECS.md (Especificações Técnicas)
...
- **Inputs:**
  - `PRD.md` validado (Fase 1 — obrigatório)
  - `ARCHITECTURE.md` validado (Fase 2 — obrigatório)
  - `SECURITY.md` validado (Fase 3 — obrigatório)
- **Checkpoint de Rastreabilidade:** Cada especificação funcional deve rastrear de volta a uma feature ou user story do PRD.md. Cada NFR deve estar vinculado a um ADR do ARCHITECTURE.md.
- **Responsáveis:** Tech Lead / Equipe de Desenvolvimento
- **Entregável:** Arquivo `SPECS.md` em `{SPECS_PATH}/SPECS.md` validado pelo humano com marcador `[STATUS: COMPLIANCE]`
- **Pipeline Sequencial:**
  1. Executar `PROMPT-GENERATE-SPECS-TECHNICAL_SOLUTION.md` com parâmetros: `{SOLUTION_PATH=TECHNICAL_SOLUTION_COMPLETE_PATH_NAME}`, `{PROJECT_PATH=PROJECT_COMPLETE_PATH_NAME}`, `{PROJECT_NAME=PROJECT_ID_NAME}`, `{SOLUTION_NAME=TECHNICAL_SOLUTION_NAME}`, `{SOLUTION_TYPE=backend}`, `{SCOPE=full}`
  2. Validar via `PROMPT-GATE-SPECS-TECHNICAL_SOLUTION.md`
  3. Aplicar loops de correção (`PROMPT-FIX-SPECS-TECHNICAL_SOLUTION.md`) ou retrocesso até o aceite humano final.
  4. ⚠️ **Atenção ao efeito cascata:** Se SPECS.md for modificado, TASKS.md e TEST_PLAN.md devem ser regenerados.

---

### Fase 5 — TASKS.md (Tarefas de Implementação)
...
- **Inputs:**
  - `PRD.md` validado (Fase 1)
  - `ARCHITECTURE.md` validado (Fase 2)
  - `SPECS.md` validado (Fase 4 — obrigatório)
- **Checkpoint de Rastreabilidade:** Toda tarefa deve rastrear de volta a uma especificação do SPECS.md. Nenhuma tarefa "órfã" (sem especificação correspondente) é permitida.
- **Responsáveis:** Tech Lead / Equipe de Desenvolvimento
- **Entregável:** Arquivo `TASKS.md` em `{SPECS_PATH}/TASKS.md` validado pelo humano com marcador `[STATUS: COMPLIANCE]`
- **Pipeline Sequencial:**
  1. Executar `PROMPT-GENERATE-TASKS-TECHNICAL_SOLUTION.md` com parâmetros: `{SOLUTION_PATH=TECHNICAL_SOLUTION_COMPLETE_PATH_NAME}`, `{PROJECT_PATH=PROJECT_COMPLETE_PATH_NAME}`, `{PROJECT_NAME=PROJECT_ID_NAME}`, `{SOLUTION_NAME=TECHNICAL_SOLUTION_NAME}`, `{SCOPE=full}`
  2. Validar via `PROMPT-GATE-TASKS-TECHNICAL_SOLUTION.md`
  3. Aplicar loops de correção (`PROMPT-FIX-TASKS-TECHNICAL_SOLUTION.md`) ou retrocesso até o aceite humano final.
  4. ⚠️ **Atenção ao efeito cascata:** Se TASKS.md for modificado, TEST_PLAN.md deve ser regenerado.

---

### Fase 6 — TEST_PLAN.md (Plano de Testes)
...
- **Inputs:**
  - `PRD.md` validado (Fase 1)
  - `ARCHITECTURE.md` validado (Fase 2)
  - `SPECS.md` validado (Fase 4)
  - `TASKS.md` validado (Fase 5 — obrigatório)
- **Checkpoint de Rastreabilidade Mestre:** Executar uma validação cruzada completa — cada cenário de teste deve rastrear de volta a uma user story do PRD.md, e cada controle de segurança testado deve corresponder a um item do SECURITY.md. A cadeia completa é: `TEST_PLAN → TASKS → SPECS → SECURITY → ARCHITECTURE → PRD → Project Charter`.
- **Responsáveis:** QA Engineer / Tech Lead
- **Entregável:** Arquivo `TEST_PLAN.md` em `{SPECS_PATH}/TEST_PLAN.md` validado pelo humano com marcador `[STATUS: COMPLIANCE]`
- **Pipeline Sequencial:**
  1. Executar `PROMPT-GENERATE-TEST_PLAN-TECHNICAL_SOLUTION.md` com parâmetros: `{SOLUTION_PATH=TECHNICAL_SOLUTION_COMPLETE_PATH_NAME}`, `{PROJECT_PATH=PROJECT_COMPLETE_PATH_NAME}`, `{PROJECT_NAME=PROJECT_ID_NAME}`, `{SOLUTION_NAME=TECHNICAL_SOLUTION_NAME}`, `{STACK=STACK_DEFINITION}`, `{SCOPE=full}`
  2. Validar via `PROMPT-GATE-TEST_PLAN-TECHNICAL_SOLUTION.md`
  3. Aplicar loops de correção (`PROMPT-FIX-TEST_PLAN-TECHNICAL_SOLUTION.md`) ou retrocesso até o aceite humano final.

---

## MAPEAMENTO DE RASTREABILIDADE (FASE 6 vs FASES 1-5)

Antes de aprovar a conclusão da Fase 6 (TEST_PLAN.md), a IA deverá obrigatoriamente executar uma validação cruzada completa:

1. **Mapeamento de Dependências:** Cada cenário de teste deve, obrigatoriamente, estar vinculado retroativamente: Cenário de Teste → Tarefa (TASKS.md) → Especificação (SPECS.md) → Controle de Segurança (SECURITY.md) → Decisão Arquitetural (ARCHITECTURE.md) → Requisito de Produto (PRD.md) → Objetivo de Negócio (Project Charter).
2. **Identificação de Órfãos:** Alertar o usuário se existir algum cenário de teste que NÃO possua um requisito correspondente no PRD.md (evitando escopo oculto).
3. **Verificação de Cobertura:** Garantir que 100% dos requisitos funcionais e não-funcionais definidos no PRD.md e SPECS.md foram cobertos por pelo menos um cenário de teste.
4. **Verificação de Segurança:** Garantir que 100% dos controles definidos no SECURITY.md possuem cenários de teste correspondentes no TEST_PLAN.md.
...
|:---|:---|:---|:---|:---|
| 1 | PRD.md | `PROMPT-GENERATE-PRD-TECHNICAL_SOLUTION.md` | `PROMPT-GATE-PRD-TECHNICAL_SOLUTION.md` | `PROMPT-FIX-PRD-TECHNICAL_SOLUTION.md` |
| 2 | ARCHITECTURE.md | `PROMPT-GENERATE-ARCHITECTURE-TECHNICAL_SOLUTION.md` | `PROMPT-GATE-ARCHITECTURE-TECHNICAL_SOLUTION.md` | `PROMPT-FIX-ARCHITECTURE-TECHNICAL_SOLUTION.md` |
| 3 | SECURITY.md | `PROMPT-GENERATE-SECURITY-TECHNICAL_SOLUTION.md` 🆕 | `PROMPT-GATE-SECURITY-TECHNICAL_SOLUTION.md` 🆕 | `PROMPT-FIX-SECURITY-TECHNICAL_SOLUTION.md` 🆕 |
| 4 | SPECS.md | `PROMPT-GENERATE-SPECS-TECHNICAL_SOLUTION.md` | `PROMPT-GATE-SPECS-TECHNICAL_SOLUTION.md` | `PROMPT-FIX-SPECS-TECHNICAL_SOLUTION.md` |
| 5 | TASKS.md | `PROMPT-GENERATE-TASKS-TECHNICAL_SOLUTION.md` | `PROMPT-GATE-TASKS-TECHNICAL_SOLUTION.md` | `PROMPT-FIX-TASKS-TECHNICAL_SOLUTION.md` |
| 6 | TEST_PLAN.md | `PROMPT-GENERATE-TEST_PLAN-TECHNICAL_SOLUTION.md` | `PROMPT-GATE-TEST_PLAN-TECHNICAL_SOLUTION.md` | `PROMPT-FIX-TEST_PLAN-TECHNICAL_SOLUTION.md` |
...
|:---|:---|:---|:---|
| 1.0 | 21/07/2026 | Criação inicial: roadmap completo de 6 fases com bootstrap inteligente, SECURITY.md como novo artefato, integração com GLOBAL-SECURITY.md e ARCHITECTURE_GLOBAL, efeitos cascata documentados | Time de Arquitetura |
