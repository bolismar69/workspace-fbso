...
Este prompt gera o artefato `UPSTREAM-ARCHITECTURE-DISCOVERY-ARCHITECTURE-DEFINITION.md` — a **definição de arquitetura do projeto** que especifica como todas as soluções técnicas se integram. Este documento **absorve e substitui** o INTEGRATION-MAP.md existente (criado ad-hoc) e define o "C4 Level 1 e 2 do projeto inteiro".

**Inputs upstream:** `UPSTREAM-ARCHITECTURE-DISCOVERY-SOLUTIONS-CATALOG.md` (Fase 2) + `UPSTREAM-ARCHITECTURE-DISCOVERY-PRD-DEFINITION.md` (Fase 4) + `architecture/` (ADRs, blueprints, data standards globais) + TECHNICAL-PLAN.md (referência).
...
Gerar `{TECHNICAL_DEFINITIONS_PATH}/UPSTREAM-ARCHITECTURE-DISCOVERY-ARCHITECTURE-DEFINITION.md` com:
