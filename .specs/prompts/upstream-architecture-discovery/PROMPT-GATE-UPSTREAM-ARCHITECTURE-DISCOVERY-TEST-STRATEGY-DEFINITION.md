...
Este prompt implementa o **Gate de Validação da Definição de Estratégia de Testes** para o artefato `UPSTREAM-ARCHITECTURE-DISCOVERY-TEST-STRATEGY-DEFINITION.md`. Verifica se a estratégia de testes do projeto está completa, cobre a pirâmide de testes, automação, performance, segurança e quality gates, e está alinhada com a arquitetura, segurança e DevOps.
...
**Arquivos gerados pelo GENERATE:** `UPSTREAM-ARCHITECTURE-DISCOVERY-TEST-STRATEGY-DEFINITION.md`
...
Ler `UPSTREAM-ARCHITECTURE-DISCOVERY-TEST-STRATEGY-DEFINITION.md`, Architecture Definition (F7), Security Definition (F8), DevOps SRE (F10), Infra Cloud (F12), ADRs globais.
