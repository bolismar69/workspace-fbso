...
2. **Seção 1 — Alocação:** Toda alocação referencia perfil do 062 (STF-NN) e fila do 092 (FILA-NN)? Desvios vs baseline declarados?
3. **Seção 2 — Capacidade:** Capacidade baseline, demanda das filas, saldo e ação calculados para cada perfil? Nenhuma sobrecarga sem ação?
4. **Seção 3 — Impedimentos:** Todo impedimento (IMP-NN) tem tipo, impacto em fila/entrega, plano de ação, responsável e status?
...
7. **Vocabulário WATERFALL:** Respeita a tabela VOCABULÁRIO WATERFALL do GENERATE? IDs usam apenas STF-NN/IMP-NN/FILA-NN?
