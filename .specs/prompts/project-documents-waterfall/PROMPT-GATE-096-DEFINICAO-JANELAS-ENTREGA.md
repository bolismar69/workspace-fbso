...
6. **Matriz de Transição:** o loop DEV→QA→UAT→DEPLOY→DEV por CICLO-NN está diagramado com as 3 tratativas de retorno (QA NO-GO, UAT divergência via 085, DEPLOY bloqueado via IMP-NN)? — `[096-05]`
...
10. **Limite de Escopo (fronteira dupla):** o documento NÃO define Ciclos de Entrega (`CICLO-NN` — exclusivo do 092)? — `[096-09]`
