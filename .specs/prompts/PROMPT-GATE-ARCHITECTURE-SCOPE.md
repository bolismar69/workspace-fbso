...
Este prompt implementa o **Gate de Alinhamento de Escopo** para o artefato `ARCHITECTURE.md`, conforme definido no fluxo Spec-Driven Development (etapa 2.1 — GE3).

O agente validador verifica se a arquitetura proposta está **aderente ao escopo definido no PRD.md** e **não introduz complexidade ou componentes além do necessário** para atender aos requisitos. O foco aqui é scope creep técnico — arquitetura que vai além do que o PRD.md demanda.

**Princípio fundamental:** O ARCHITECTURE.md define o COMO. Se o COMO for mais complexo ou abrangente do que o OQUE (definido no PRD.md), há scope creep técnico.
...
Ler obrigatoriamente:
    ├── {SOLUTION_PATH}/.specs/business-projects/{PROJECT_NAME}/ARCHITECTURE.md (artefato a ser validado)
    ├── {SOLUTION_PATH}/.specs/business-projects/{PROJECT_NAME}/PRD.md (baseline de escopo — DEVE existir e estar APROVADO)
    └── Documentos de referência do projeto em {PROJECT_PATH}:
          ├── TECHNICAL-PLAN.md (stack, ERD, ADRs globais, sequenciamento)
          ├── ARCHITECTURE.md (visão arquitetural do projeto — C4, ADRs)
          └── INTEGRATION-MAP.md (integrações, fluxos de dados)

Se ARCHITECTURE.md não existir → ERRO: "ARCHITECTURE.md não encontrado. Execute o agente gerador de arquitetura."
Se PRD.md não existir → ERRO: "PRD.md não encontrado. O gate de arquitetura depende do PRD.md aprovado."
```

### Passo 2 — Executar Dimensões de Validação de Escopo

O gate avalia o ARCHITECTURE.md em **5 dimensões**. Para cada dimensão, atribuir um veredito: `APROVADO`, `RESSALVA` ou `REPROVADO`.

#### Dimensão 1: Aderência ao PRD.md (Scope Baseline Técnico)

| # | Verificação | Critério |
|---|---|---|
| 1.1 | Entidades mapeadas | As entidades descritas no ARCHITECTURE.md correspondem exatamente às entidades listadas no PRD.md? Há entidades extras não solicitadas? |
| 1.2 | Funcionalidades cobertas | Cada feature do PRD.md tem um componente/serviço correspondente na arquitetura? Há componentes para features inexistentes? |
| 1.3 | NFRs atendidos | Os requisitos não-funcionais do PRD.md estão refletidos nas decisões arquiteturais? (ex: multi-tenancy, RBAC, auditoria) |
...
| # | Verificação | Critério |
|---|---|---|
| 2.1 | Package-by-Layer vs Clean Architecture | Se o PRD.md não justifica Clean/Hexagonal, a arquitetura usa package-by-layer? |
| 2.2 | Número de camadas | A arquitetura tem mais camadas do que o estritamente necessário? (suspeito: > 4 camadas) |
| 2.3 | Número de serviços externos | A arquitetura introduz serviços/infraestrutura não exigidos pelo PRD.md? (ex: Kafka sem requisito de streaming) |
| 2.4 | Complexidade de padrões | Há padrões de design que adicionam complexidade sem justificativa no PRD.md? (ex: CQRS, Event Sourcing sem necessidade) |

#### Dimensão 3: Consistência com Technical Plan e ADRs Globais

| # | Verificação | Critério |
|---|---|---|
| 3.1 | Stack tecnológica | A stack do ARCHITECTURE.md é consistente com o TECHNICAL-PLAN.md do projeto? |
...
| # | Verificação | Critério |
|---|---|---|
| 4.1 | Estrutura de pacotes | A árvore de diretórios cobre todos os componentes necessários para as features do PRD.md? |
...
| # | Verificação | Critério |
|---|---|---|
| 5.1 | Tenant isolation | O mecanismo de isolamento multi-tenant está definido e é adequado ao PRD.md? |
| 5.2 | RBAC | O controle de acesso baseado em papéis está modelado conforme requisitos do PRD.md? |
| 5.3 | Auditoria | O mecanismo de auditoria cobre as operações exigidas pelo PRD.md? |
...
```markdown
# ARCHITECTURE_SCOPE_FAIL_REPORT.md — Relatório de Validação de Escopo: ARCHITECTURE.md

[Header com metadados: solução, projeto, stack, data da validação, versão do ARCHITECTURE.md validado]
...
### 3.1 [Dimensão 1] Aderência ao PRD.md
| ID | Verificação | Status | Evidência | Ação Corretiva |
|---|---|---|---|---|
| NC-001 | 1.1 Entidades mapeadas | REPROVADO | Entidade "NotificationLog" na arquitetura sem correspondência no PRD.md | Remover ou justificar no PRD.md |
...
- Componentes/serviços na arquitetura sem feature correspondente no PRD.md
...
- Seções do ARCHITECTURE.md que precisam ser simplificadas ou removidas

## 6. Próximos Passos
1. Encaminhar este relatório ao Agente Arquiteto para correção
2. Após correção, reexecutar este gate (e o gate de PRD.md se houve alteração de escopo)
3. Não prosseguir para SPECS.md até ARCHITECTURE.md ser APROVADO
...
| 2ª | `gap-analysis` | Análise de lacunas entre ARCHITECTURE.md e PRD.md |
...
1. **O PRD.md é a constituição do escopo.** Toda verificação de escopo do ARCHITECTURE.md é feita contra o PRD.md aprovado. Se o PRD.md não passou pelo gate anterior, este gate não deve ser executado.

2. **Simplicidade é requisito, não preferência.** A dimensão 2 (KISS) não é opcional. O skill `ponytail` é invocado como revisor mandatório. Complexidade sem justificativa documentada no PRD.md é tratada como não-conformidade.
...
4. **O relatório de falha referencia o PRD.md.** As ações corretivas devem citar exatamente qual seção do PRD.md justifica (ou não) cada decisão arquitetural.

5. **Revalidação em cascata.** Se a correção do ARCHITECTURE.md alterar o escopo, o gate de PRD.md deve ser reexecutado para garantir consistência bidirecional.
...
|:---|:---|:---|:---|
| 1.0 | 13/07/2026 | Criação inicial: gate de escopo para ARCHITECTURE.md, 5 dimensões, 20 verificações, foco em scope creep técnico e simplicidade | Time de Arquitetura |
