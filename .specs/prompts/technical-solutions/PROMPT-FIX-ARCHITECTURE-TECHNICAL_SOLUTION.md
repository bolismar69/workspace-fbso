...
Este prompt é acionado quando o **Gate de Alinhamento de Escopo** (PROMPT-GATE-ARCHITECTURE-SCOPE) reprova o `TECHNICAL-SOLUTION-ARCHITECTURE.md` e gera o relatório `ARCHITECTURE_SCOPE_FAIL_REPORT.md`.

O agente atua como **corretor de arquitetura** — lê o relatório de falha e aplica correções cirúrgicas no TECHNICAL-SOLUTION-ARCHITECTURE.md, com foco especial em **simplificação** (remoção de complexidade desnecessária) e **alinhamento ao TECHNICAL-SOLUTION-PRD.md**.
...
Ler obrigatoriamente:
    ...
    ├── {SOLUTION_PATH}/.specs/business-projects/{PROJECT_NAME}/TECHNICAL-SOLUTION-ARCHITECTURE.md (a corrigir)
    ├── {SOLUTION_PATH}/.specs/business-projects/{PROJECT_NAME}/TECHNICAL-SOLUTION-PRD.md (baseline de escoco)
...
|---|---|---|
| P0 (Crítica) | Scope creep técnico (componentes/serviços sem feature no TECHNICAL-SOLUTION-PRD.md) | **Remover** componente da arquitetura |
...
| P1 (Alta) | Entidade/componente sem correspondência no TECHNICAL-SOLUTION-PRD.md | **Remover** ou **justificar no TECHNICAL-SOLUTION-PRD.md** |
...
### Passo 3 — Aplicar Correções no TECHNICAL-SOLUTION-ARCHITECTURE.md
...
Para cada NC no relatório:
    ...
    │     → Consultar documento de referência (TECHNICAL-SOLUTION-PRD.md, TECHNICAL-PLAN.md)
    ...
    └── NC de COMPLEMENTO (incompleto)?
          ...
          → Gerar conteúdo seguindo padrão do TECHNICAL-SOLUTION-ARCHITECTURE.md
...
```
Invocar ponytail sobre o TECHNICAL-SOLUTION-ARCHITECTURE.md corrigido:
...
| 5 | Consistência com TECHNICAL-SOLUTION-PRD.md | Arquitetura cobre exatamente o escopo do TECHNICAL-SOLUTION-PRD.md |
...
| 5ª | `documentation-writer` | Qualidade do TECHNICAL-SOLUTION-ARCHITECTURE.md corrigido |
...
2. **Remover > Mover para "Futuro".** Se um componente não é justificado pelo TECHNICAL-SOLUTION-PRD.md atual, ele deve ser REMOVIDO, não movido para uma seção de "trabalho futuro". A seção "Futuro" só deve conter itens explicitamente mencionados no TECHNICAL-SOLUTION-PRD.md como fases posteriores.

3. **Cada remoção deve ser documentada.** Quando um componente é removido, documentar no ADR de simplificação o motivo da remoção e a referência ao TECHNICAL-SOLUTION-PRD.md que justifica a decisão.

4. **Incrementar versão SEMPRE.** Toda correção gera nova versão com entrada no registro de alterações.

5. **Após correção, reexecutar o gate.** FIX → GATE → (se APROVADO) prosseguir. Se o escopo foi alterado, reexecutar também o gate de TECHNICAL-SOLUTION-PRD.md.
...
|:---|:---|:---|:---|
| 1.0 | 13/07/2026 | Criação inicial: correção de TECHNICAL-SOLUTION-ARCHITECTURE.md baseada em ARCHITECTURE_SCOPE_FAIL_REPORT.md, foco em simplificação e remoção de scope creep técnico | Time de Arquitetura |
