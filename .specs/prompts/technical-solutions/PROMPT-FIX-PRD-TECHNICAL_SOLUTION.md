...
Este prompt é acionado quando o **Gate de Alinhamento de Escopo** (PROMPT-GATE-PRD-SCOPE) reprova o `TECHNICAL-SOLUTION-PRD.md` e gera o relatório `PRD_SCOPE_FAIL_REPORT.md`.

O agente atua como **corretor de escopo** — lê o relatório de falha, consome cada não-conformidade reportada e corrige o TECHNICAL-SOLUTION-PRD.md para que ele passe no gate na próxima execução.

**Princípio fundamental:** Este prompt NÃO reescreve o TECHNICAL-SOLUTION-PRD.md do zero. Ele aplica correções cirúrgicas baseadas no relatório de falha, preservando o conteúdo que já está correto.
...
Ler obrigatoriamente:
    ...
    ├── {SOLUTION_PATH}/.specs/business-projects/{PROJECT_NAME}/TECHNICAL-SOLUTION-PRD.md (artefato a ser corrigido)
...
Se TECHNICAL-SOLUTION-PRD.md não existir → ERRO: "TECHNICAL-SOLUTION-PRD.md não encontrado."
```
...
|---|---|---|
| P0 (Crítica) | Scope creep confirmado (features/BRs sem origem nos docs de negócio) | **Remover** do TECHNICAL-SOLUTION-PRD.md |
| P1 (Alta) | Feature/entrega do Project Charter não mapeada no TECHNICAL-SOLUTION-PRD.md | **Adicionar** seção correspondente |
...
### Passo 3 — Aplicar Correções no TECHNICAL-SOLUTION-PRD.md
...
Para cada NC no relatório (§3):
    ...
    │     → Localizar a feature/BR/requisito no TECHNICAL-SOLUTION-PRD.md
    ...
    │     → Adicionar seção/feature/BR no local apropriado do TECHNICAL-SOLUTION-PRD.md
    │     → Atualizar matriz de rastreabilidade
    │
...
Adicionar entrada no registro de alterações do TECHNICAL-SOLUTION-PRD.md:
...
| 6 | Consistência cross-documento | TECHNICAL-SOLUTION-PRD.md corrigido é consistente com Project Charter, BRD, Épicos e Features |
...
| 3ª | `agile-ba-practices` | Boas práticas de BA na escrita do TECHNICAL-SOLUTION-PRD.md |
| 4ª | `documentation-writer` | Qualidade e consistência do TECHNICAL-SOLUTION-PRD.md corrigido |

---

## Observações

1. **Correções cirúrgicas, não reescrita.** O objetivo é resolver as não-conformidades apontadas no relatório, não reescrever o TECHNICAL-SOLUTION-PRD.md. Se o relatório indicar que uma reescrita completa é necessária, usar o PROMPT-GENERATE-PRD-ARTEFACT com `SCOPE=full`.
...
3. **O relatório de falha é o contrato.** Cada correção deve ser rastreável a uma NC específica do relatório. Não fazer alterações não solicitadas no TECHNICAL-SOLUTION-PRD.md durante a correção.

4. **Incrementar versão SEMPRE.** Toda correção gera uma nova versão do TECHNICAL-SOLUTION-PRD.md com entrada no registro de alterações referenciando o relatório de falha.

5. **Após correção, reexecutar o gate.** O fluxo natural é: FIX PRD → GATE PRD → (se APROVADO) prosseguir. Não considerar o TECHNICAL-SOLUTION-PRD.md como aprovado até o gate confirmar.
...
|:---|:---|:---|:---|
| 1.0 | 13/07/2026 | Criação inicial: correção de TECHNICAL-SOLUTION-PRD.md baseada em PRD_SCOPE_FAIL_REPORT.md, 4 níveis de prioridade, 6 verificações pós-correção | Time de Arquitetura |
