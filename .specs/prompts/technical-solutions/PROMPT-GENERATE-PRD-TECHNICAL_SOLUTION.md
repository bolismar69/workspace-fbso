...
Este prompt orquestra skills especializadas em análise de negócio, requisitos e product management para gerar ou revisar o artefato `TECHNICAL-SOLUTION-PRD.md` (Product Requirements Document) na pasta de especificações de uma solução técnica.

O artefato gerado é a **porta de entrada para a documentação de negócio** — um sumário executivo de alto nível que referencia os documentos-fonte do projeto (Project Charter, Business Requirements, Épicos, Features, User Stories) e serve como baseline de escopo para toda a cadeia downstream: TECHNICAL-SOLUTION-ARCHITECTURE.md → TECHNICAL-SOLUTION-SPECS.md → TECHNICAL-SOLUTION-TASKS.md → TECHNICAL-SOLUTION-TEST_PLAN.md.

**Princípio fundamental:** O TECHNICAL-SOLUTION-PRD.md não duplica os documentos de negócio. Ele os **resume, referencia e contextualiza** para o time técnico, estabelecendo o escopo que será validado pelo GATE-PRD-SCOPE e desdobrado em arquitetura e especificações.
...
| `{BRANCH_STRATEGY}` | Estratégia de branching do projeto: `branch-por-sprint` (múltiplas sprints, uma branch `feature/sprint-NN-<slug>` por sprint) ou `branch-unica` (sprint único, branch `feature/<nome-projeto>`). A seção de branching do TECHNICAL-SOLUTION-PRD.md gerado deve documentar a estratégia completa com tabela, workflow git e comandos. |
...
### Passo 3 — Gerar ou Atualizar o Artefato TECHNICAL-SOLUTION-PRD.md
...
|---|---|---|
| 1 | Arquivo no path correto | `{SOLUTION_PATH}/.specs/business-projects/{PROJECT_NAME}/TECHNICAL-SOLUTION-PRD.md` existe |
...
- Não existe TECHNICAL-SOLUTION-PRD.md prévio — criar do zero
...
- TECHNICAL-SOLUTION-PRD.md já existe — atualizar apenas seções que mudaram
- Identificar o delta: quais BRs, features, épicos ou KPIs foram adicionados/alterados?
- Preservar conteúdo não afetado
- Incrementar versão (ex: 1.0 → 1.1) e documentar alterações no próprio TECHNICAL-SOLUTION-PRD.md
...
|---|---|---|---|
| 1ª | `prd` | Estrutura e template do TECHNICAL-SOLUTION-PRD.md | full, delta |
...
```
Humano: "Gerar TECHNICAL-SOLUTION-PRD.md para o ms-billing-engine-tax-rates"
...
1. **TECHNICAL-SOLUTION-PRD.md é um sumário, não uma duplicata.** O TECHNICAL-SOLUTION-PRD.md NÃO repete o conteúdo completo dos documentos de negócio. Cada seção resume os pontos principais e referencia o documento-fonte exato (com link relativo e indicação de seção) onde os detalhes podem ser encontrados.
...
5. **O TECHNICAL-SOLUTION-PRD.md evolui com o projeto.** Na Fase 0 (Fundação), ele é um esboço baseado no Project Charter. Conforme as ondas são detalhadas (Onda 1, Onda 2), ele deve ser atualizado com novos BRs, features e KPIs (modo `delta`).

6. **O aviso de leitura é obrigatório.** O bloco ⚠️ no header é essencial para estabelecer expectativas corretas: este documento é um resumo, não a especificação completa. Isso evita que o time técnico tome decisões baseadas apenas no TECHNICAL-SOLUTION-PRD.md sem consultar os docs-fonte.

7. **O TECHNICAL-SOLUTION-PRD.md é o primeiro artefato da cadeia.** Ele alimenta TECHNICAL-SOLUTION-ARCHITECTURE.md (que define o COMO), que alimenta TECHNICAL-SOLUTION-SPECS.md (que define O QUE construir), que alimenta TECHNICAL-SOLUTION-TASKS.md e TECHNICAL-SOLUTION-TEST_PLAN.md. Um TECHNICAL-SOLUTION-PRD.md incorreto contamina toda a cadeia. Por isso o GATE-PRD-SCOPE é o primeiro gate do fluxo.
