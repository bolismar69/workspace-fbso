...
Este prompt orquestra skills especializadas em especificação de solução e documentação técnica para gerar ou revisar o artefato `TECHNICAL-SOLUTION-SPECS.md` na pasta de especificações de uma solução técnica.
...
Verificar se existe: {SOLUTION_PATH}/.specs/business-projects/{PROJECT_NAME}/
    ...
    └── SIM, existe →
            │
            ├── Verificar se existem TECHNICAL-SOLUTION-PRD.md E TECHNICAL-SOLUTION-ARCHITECTURE.md na pasta
            │     │
            │     ├── AMBOS existem → Ler TECHNICAL-SOLUTION-PRD.md + TECHNICAL-SOLUTION-ARCHITECTURE.md como ponto de partida
            │     │
            │     ├── Apenas TECHNICAL-SOLUTION-PRD.md → Ler TECHNICAL-SOLUTION-PRD.md + documentos do projeto
            │     │
            │     ├── Apenas TECHNICAL-SOLUTION-ARCHITECTURE.md → Ler TECHNICAL-SOLUTION-ARCHITECTURE.md + documentos do projeto
            │     │
            ...
            │           ├── TECHNICAL-SOLUTION-ARCHITECTURE.md (C4, ADRs, pipeline de segurança)
            │           ├── API-CONTRACTS.md (contratos de API — para backend)
            │           └── DEFINITION_OF_DONE.md (critérios de DONE)
            │
            └── Verificar se TECHNICAL-SOLUTION-SPECS.md já existe:
                  │
...
|---|---|---|---|
| 1ª | `create-specification` | Criação de especificação funcional e técnica | Estrutura do TECHNICAL-SOLUTION-SPECS.md, mapeamento requisitos → features |
...
### Passo 3 — Gerar ou Atualizar o Artefato TECHNICAL-SOLUTION-SPECS.md

Gerar o arquivo em:
```
{SOLUTION_PATH}/.specs/business-projects/{PROJECT_NAME}/TECHNICAL-SOLUTION-SPECS.md
```

#### Estrutura Obrigatória do Arquivo

```markdown
# TECHNICAL-SOLUTION-SPECS.md — Especificação da Solução: {SOLUTION_NAME}
...
|---|---|---|
| 1 | Arquivo no path correto | `{SOLUTION_PATH}/.specs/business-projects/{PROJECT_NAME}/TECHNICAL-SOLUTION-SPECS.md` existe |
...
- Não existe TECHNICAL-SOLUTION-SPECS.md prévio — criar do zero
- Ler todos os documentos disponíveis (TECHNICAL-SOLUTION-PRD.md, TECHNICAL-SOLUTION-ARCHITECTURE.md, docs do projeto)
- Invocar todas as 5 skills

### SCOPE = delta (Atualização Parcial)

- Executar Passos 0→1→2→3→4
- TECHNICAL-SOLUTION-SPECS.md já existe — atualizar apenas seções que mudaram
...
|---|---|---|---|
| 1ª | `create-specification` | Estrutura e conteúdo do TECHNICAL-SOLUTION-SPECS.md | full, delta |
...
```
Humano: "Gerar TECHNICAL-SOLUTION-SPECS.md para o backend ms-fbso-platform-admin"
...
1. **TECHNICAL-SOLUTION-PRD.md + TECHNICAL-SOLUTION-ARCHITECTURE.md são o ponto de partida preferencial.** Se ambos existirem, eles contêm o escopo, entidades, ADRs e estrutura de pacotes. Os documentos do projeto são complementares para rastreabilidade.

2. **TECHNICAL-SOLUTION-SPECS.md é a ponte negócio ↔ técnico.** Ele não repete o TECHNICAL-SOLUTION-PRD.md (que é um guia de entrada) nem o TECHNICAL-SOLUTION-ARCHITECTURE.md (que define o como). O TECHNICAL-SOLUTION-SPECS.md define **o que** precisa ser construído com precisão suficiente para o time começar a codificar.

3. **Rastreabilidade é obrigatória.** Toda especificação deve referenciar o requisito de negócio (BR), a feature (FXX-XX) e a user story (US-XXX) que a originou. Sem rastreabilidade, o TECHNICAL-SOLUTION-SPECS.md vira um documento isolado que ninguém confia.
...
6. **O TECHNICAL-SOLUTION-SPECS.md evolui com o projeto.** Na Fase 0 (Fundação), ele é um esboço. Conforme as features são implementadas (M2→M6), ele deve ser atualizado com o que foi efetivamente construído (modo `delta`).
...
|:---|:---|:---|:---|
| 1.1 | 16/07/2026 | Removido `{BRANCH_NAME}` — informação de branch é documentada no TECHNICAL-SOLUTION-PRD.md e SPRINT-CARD.md, não no TECHNICAL-SOLUTION-SPECS.md. Header do template sem linha `Branch:`. Parâmetros reduzidos de 7 para 6. | Time Técnico |
