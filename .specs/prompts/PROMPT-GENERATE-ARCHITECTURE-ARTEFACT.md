...
Este prompt orquestra skills especializadas em arquitetura de solução e engenharia de software para gerar ou revisar o artefato `ARCHITECTURE.md` na pasta de especificações de uma solução técnica.
...
Verificar se existe: {SOLUTION_PATH}/.specs/business-projects/{PROJECT_NAME}/
    ...
    └── SIM, existe →
            ...
            │     │
            │     ├── SIM → Ler PRD.md como ponto de partida principal
            ...
            │
            └── Ler também (sempre, independente de PRD.md):
...
### Passo 3 — Gerar ou revisar o Artefato ARCHITECTURE.md
...
```markdown
# ARCHITECTURE.md — Arquitetura da Solução: {SOLUTION_NAME}
...
|---|---|---|
| 1 | Arquivo criado no path correto | `{SOLUTION_PATH}/.specs/business-projects/{PROJECT_NAME}/ARCHITECTURE.md` existe |
...
```
Humano: "Gerar ARCHITECTURE.md para o microsserviço ms-fbso-platform-admin"
...
1. **PRD.md é o ponto de partida preferencial.** Se existir, ele contém o escopo exato do que o microsserviço implementa, as entidades, ADRs e a estrutura de pacotes esperada. Os documentos do projeto são complementares.
...
3. **Package-by-layer é o padrão default.** A menos que o PRD.md ou os documentos do projeto justifiquem explicitamente uma arquitetura mais complexa (Clean, Hexagonal), o padrão é package-by-layer com aspectos AOP para cross-cutting concerns.

4. **Nunca sobrescrever um ARCHITECTURE.md existente sem auditoria de delta.** Se o arquivo já existir, gerar uma nova versão (incrementar versão) e documentar no Registro de Alterações o que mudou e por quê.
