...
2. O arquivo individual da história que apresentou falha (Exemplo: `/user-stories/US-FEAT-EP-0001-0001-0001-dashboard-metricas-operacionais.md`, seguindo o padrão `US-FEAT-{codigo-feature}-{SSSS}-{nome-da-user-story}.md` onde `SSSS` é sequencial global 0001-9999).
...
1. **Isolamento de Arquivo (Escrita Atômica):** Você NÃO deve reescrever outros arquivos da pasta `/user-stories/`. Sua atuação está estritamente limitada ao arquivo específico da história apontado na falha (Ex: `US-FEAT-{codigo-feature}-{SSSS}-{nome-da-user-story}.md`). Mantenha os demais documentos atômicos intactos.
2. **Manutenção do Índice Mestre (RTM):** Se a resposta do usuário alterar o escopo, IDs ou a descrição da jornada, atualize cirurgicamente a linha correspondente a esta história dentro do arquivo `USER-STORIES-{PROJECT_ID_NAME}.md`. Certifique-se de que o link markdown relativo (`[Ver Detalhes](./user-stories/US-FEAT-{codigo-feature}-{SSSS}-{nome-da-user-story}.md)`) permaneça íntegro e funcional.
...
4. **Atualização de Versão Local:** No cabeçalho de metadados do arquivo individual da história alterada (`US-FEAT-{codigo-feature}-{SSSS}-{nome-da-user-story}.md`), incremente a sua versão específica (ex: de 1.0 para 1.1) e registre o motivo da revisão em conformidade com o ajuste do Gate 05.
